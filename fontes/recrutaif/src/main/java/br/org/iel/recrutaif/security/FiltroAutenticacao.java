...
import br.org.iel.recrutaif.rest.LoginEndpoint;
...
public class FiltroAutenticacao implements ContainerRequestFilter {
	...
	public void filter(ContainerRequestContext requestContext) throws IOException {
		...
		String autorizacaoNoHeader = requestContext.getHeaderString(HttpHeaders.AUTHORIZATION);
		...
		String token = autorizacaoNoHeader.substring("Bearer".length()).trim();
		...
		try {
			Claims claims = new LoginEndpoint().validaToken(token);
