...
import br.org.iel.recrutaif.conf.JpaUtil;
...
public class SetorREST {
	...
	public String printMessage(@PathParam("id") Integer id) {
		...
		Setor setor = new SetorDao(manager).buscaSetor(id);
		return setor.toGson();
	...
	public String mostraSetor() {
		...
		setor.setNome("TI");
		new SetorDao(manager).adiciona(setor);

		List<Setor> setores = new SetorDao(manager).listaSetores();
	...
	public Response deleteSetor(@PathParam("id") Integer id) {
		...
		new SetorDao(manager).deletaSetor(id);
		return Response.ok().build();
	}
